...
export const SAMPLE_SET: GuardrailSet = {
  id: "sample",
  name: "Agent session settlement preflight",
  purpose: "Prevent an agent session from reporting completion while continuation work is still live or state has drifted",
  workflow: "Before session settlement → inspect pending continuation/state → validate checkpoint/provider identity → allow or block settlement",
  target: "Maintainers of long-running agent and CLI runtimes",
  successDefinition:
    "A session settles only when no continuation is pending, state belongs to the current generation, checkpoints are intact, and provider/session identity is stable",
  inputMode: "checklist",
  ...
  guardrails: [
    guardrail(
      "True settlement",
      "The runtime may report idle or complete only when no runnable work, tool result, queued callback, or continuation remains.",
      {
        id: "settlement-1",
        criticality: "critical",
        enforcement: "block",
    ...
    guardrail(
      ...
      "No continuation, retry, follow-up turn, or deferred action is still expected after the proposed settlement point.",
      {
        id: "settlement-2",
        criticality: "critical",
        enforcement: "block",
    ...
    guardrail(
      "Transcript tail is committed",
      "The latest authoritative assistant/tool/user events are present in the session transcript before settlement is emitted.",
      {
        id: "settlement-3",
        criticality: "critical",
        enforcement: "block",
    ...
    guardrail(
      ...
      "Callbacks or tool results from invalidated generations must be rejected before they can change the current session.",
      {
        id: "settlement-4",
        criticality: "critical",
        enforcement: "block",
    ...
    guardrail(
      "Checkpoint survives compaction",
      "Compaction or summarization preserves the continuation checkpoint and every state field required to resume correctly.",
      {
        id: "settlement-5",
        criticality: "critical",
        enforcement: "block",
    ...
    guardrail(
      ...
      "Provider or tool waits have explicit timeout/cancellation behavior and cannot leave the session indefinitely half-settled.",
      {
        id: "settlement-6",
        criticality: "important",
        enforcement: "warn",
    ...
    guardrail(
      "Session and provider identity stay stable",
      "Resumed work is attached to the same intended session/provider identity, or an explicit migration is recorded before continuation.",
      {
        id: "settlement-7",
        criticality: "critical",
        enforcement: "block",
