...
for (const slug of actual) {
  if (!SLUG_PATTERN.test(slug)) throw new Error(`Invalid Supabase Edge Function folder name: ${slug}`);
}
if (JSON.stringify(actual) !== JSON.stringify(EXPECTED)) {
  throw new Error(`Expected Edge Function folders ${EXPECTED.join(", ")}; found ${actual.join(", ")}.`);
}
...
for (const slug of EXPECTED) {
  ...
  if (!config.includes(`[functions.${slug}]`)) throw new Error(`Missing [functions.${slug}] in supabase/config.toml`);
  ...
  if (!clientSource.includes(`"${slug}"`)) throw new Error(`Frontend constant missing literal slug ${slug}`);
...
if (!routeSource.includes("EDGE_FUNCTIONS.inferGuardrails") || !routeSource.includes("EDGE_FUNCTIONS.evaluate")) {
