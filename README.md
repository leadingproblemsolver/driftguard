...
3. Uploads Edge Function secrets and deploys both functions with handler-level user-token verification.
...
Supabase
  ...
  ├─ Postgres: RLS-protected sets, rules, and history
...
The deployable function names are literal and must be run from the repository root:
...
Deploy them separately first. This makes the failing slug explicit. The normal deploy path intentionally does **not** use `--use-api`; it uses the CLI's standard bundler. If Docker is unavailable, use the explicit fallback only after the literal commands above are confirmed:
