...
The public endpoint slugs are the literal folders `evaluate` and `infer-guardrails`. `npm run check:edge` rejects mismatches between those folders, `supabase/config.toml`, and browser invocation constants.
...
A successful call transactionally refreshes the supplied guardrail set, computes the verdict, and stores the server-authored audit record. HTTP `401`, `403`, `413`, `422`, `429`, `502`, and `504` responses include a stable error code and request ID.
