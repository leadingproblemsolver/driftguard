...
const auth = await text("supabase/functions/shared/auth.ts");
...
const http = await text("supabase/functions/shared/http.ts");
const cors = await text("supabase/functions/shared/cors.ts");
const ai = await text("supabase/functions/shared/ai.ts");
...
requireCondition(
  "RLS enabled",
  ["profiles", "guardrail_sets", "guardrails", "evaluations", "ai_request_usage"].every((table) =>
...
requireCondition(
  ...
  packageJson.devDependencies?.wrangler === "4.107.0" &&
    ...
    packageJson.scripts?.["supabase:functions"] ===
      "npx --yes supabase@2.109.0 functions deploy evaluate infer-guardrails --no-verify-jwt" &&
...
requireCondition(
  ...
  browser.includes("Stop agent sessions from settling in the wrong state.") &&
...
requireCondition(
  ...
  browser.includes("Step 1 · Intent") &&
    ...
    browser.includes("Advanced control"),
