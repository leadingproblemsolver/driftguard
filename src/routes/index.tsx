...
function Index() {
  ...
  const [checkText, setCheckText] = useState(
    "The agent emitted idle after the model turn, but a queued tool callback can still append a result and schedule another continuation.",
  );
    ...
    <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-24 pt-20 sm:px-8 lg:grid-cols-[1.02fr_.98fr] lg:items-center lg:px-10 lg:pb-28 lg:pt-28">
          <div>
            <div className="eyebrow">
              <ShieldCheck size={15} />
              Preflight control for agent runtimes
            </div>
            <h1 className="mt-6 max-w-3xl text-balance font-serif text-5xl font-semibold leading-[0.98] tracking-[-0.045em] sm:text-6xl lg:text-[74px]">
              Stop agent sessions from settling in the wrong state.
            </h1>
            <p className="mt-7 max-w-2xl text-balance text-lg leading-8 text-muted-foreground sm:text-xl">
              Run a settlement preflight before your agent or CLI reports idle, complete, or safe-to-resume.
              DriftGuard checks continuation, generation, checkpoint, and identity invariants and returns
              <strong className="text-foreground"> Pass, Watch, or Block</strong> with the failing gate and smallest safe correction.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-muted-foreground">
              <span className="inline-flex items-center gap-2">
                <Check size={15} /> 7 gates preloaded
              </span>
              <span className="inline-flex items-center gap-2">
                <Check size={15} /> Built for agent / CLI runtimes
              </span>
              <span className="inline-flex items-center gap-2">
                <Check size={15} /> Deterministic block precedence
              </span>
            </div>
          </div>
      ...
      <section className="border-y border-border/70 bg-card/45">
        <div className="mx-auto grid max-w-7xl gap-0 px-5 sm:px-8 md:grid-cols-3 lg:px-10">
          <ValueStep
            number="01"
            title="Capture the settlement boundary"
            text="Paste the state transition, callback, compaction, or resume path that can falsely appear finished."
          />
          <ValueStep
            number="02"
            title="Check the seven failure gates"
            text="Continuation, transcript tail, generation invalidation, checkpoint integrity, bounded waits, and identity are explicit."
          />
          <ValueStep
            number="03"
            title="Ship the smallest safe correction"
            text="A failed blocking gate stops settlement and points to the exact invariant that must be repaired."
          />
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="eyebrow">
                <Activity size={15} />
                Agent session settlement preflight
              </div>
              <h2 className="mt-4 font-serif text-4xl font-semibold tracking-tight sm:text-5xl">
                Reproduce the failure. Run the gates. Fix the invariant.
              </h2>
              <p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">
                The ICP-specific template is already loaded. Use it immediately on a real false-idle,
                lost-continuation, stale-callback, compaction, or resume failure — then customize only what your runtime requires.
              </p>
            </div>
        ...
        <div className="space-y-5 p-5 sm:p-7">
          <div className="rounded-xl border border-border bg-background p-4">
            ...
            <p className="mt-2 text-sm leading-6">
              Mark the session idle after the model turn while a queued tool callback can still resume execution.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-[140px_1fr]">
            ...
            <div className="rounded-xl border border-border p-4">
              ...
              <p className="mt-2 text-sm leading-6">
                The session is not truly settled: continuation work can still mutate the transcript after the idle signal.
              </p>
            </div>
          </div>
          <div className="rounded-xl border border-primary/20 bg-primary/[0.035] p-4">
            ...
            <p className="mt-2 text-sm font-medium leading-6">
              Delay settlement until the callback queue is drained or invalidated for the current generation.
            </p>
          </div>
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>7 settlement gates · exact snapshot retained</span>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Prevent false settlement and continuation-state drift before it escapes into production.
          </p>
        </div>
        <button className="primary-cta" onClick={onStart}>
          Run the preflight <ArrowRight size={18} />
        </button>
      </div>
    </footer>
  );
